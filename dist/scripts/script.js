function updateClock() {
  const t = dayjs("04/08/2016 17:00:00", "DD/MM/YYYY HH:mm:ss"),
    e = dayjs(),
    n = dayjs.duration(e.diff(t)),
    s = e.diff(dayjs("04-08-2016", "DD-MM-YYYY"), "days"),
    o = n.format("HH:mm:ss"),
    i = [
      Math.random().toFixed(2),
      Math.random().toFixed(2),
      Math.random().toFixed(2),
    ];
  document.getElementById("clock").innerHTML =
    s + " days " + o + ", 1 user, load average: " + i.join(", ");
}
function uptimeCard() {
  dayjs.extend(window.dayjs_plugin_duration),
    dayjs.extend(window.dayjs_plugin_utc),
    updateClock();
  function e() {
    updateClock();
  }
  setInterval(e, 1e3);
}
uptimeCard();
var typed = new Typed(".element", {
    strings: [
      "<span style='color:#009966;'><pre>      ___     <br>     /      <br>    /::     <br>   /:/:    <br>  /:/  :   <br> /:/__/ :__<br> :   /:/  /<br>  :  /:/  / <br>   :/:/  /  <br>    ::/  /   <br>     /__/    <br></pre>",
      "<span style='color:#009966;'><pre> ██████╗██╗   ██╗██████╗ ██╗               <br>██╔════╝██║   ██║██╔══██╗██║               <br>██║     ██║   ██║██████╔╝██║               <br>██║     ██║   ██║██╔══██╗██║               <br>╚██████╗╚██████╔╝██║  ██║███████╗          <br> ╚═════╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝          <br>                                           <br>       ██╗██╗      █████╗ ██████╗ ██╗   ██╗<br>      ██╔╝██║     ██╔══██╗██╔══██╗╚██╗ ██╔╝<br>     ██╔╝ ██║     ███████║██║  ██║ ╚████╔╝ <br>    ██╔╝  ██║     ██╔══██║██║  ██║  ╚██╔╝  <br>██╗██╔╝   ███████╗██║  ██║██████╔╝   ██║   <br>╚═╝╚═╝    ╚══════╝╚═╝  ╚═╝╚═════╝    ╚═╝   <br></pre>",
      "<span style='color:#009966;'><pre>       (    (      <br>   (   ) ) ) )   <br> ( ) (()/((()/(   <br> )((_) /(_))/(_))  <br>((_)_ (_)) (_))_   <br> | _ )/ __| |     <br> | _ \\__  | |) | <br> |___/|___/ |___/ <br></pre>",
      "<span style='color:#009966;'><pre>___/~~____/~~~~~___/~~~~~~__/~~~~~ <br>__/~~____/~~_/~~_/~~_/~~_/~~_/~~   <br>_/~~~~~___/~~~~~_____/~~~___/~~~~~<br>/~~_/~~_/~~_/~~_/~~_/~~_/~~_/~~   <br>_/~~~~~___/~~~~~___/~~~~~~__/~~~~~   <br>_/~~~~~____/~~_____/~~~~~_____/~~~~~<br>/~~_/~~__/~~~____/~~_/~~___/~~_/~~<br>_/~~~~~____/~~___/~~___/~~_/~~___/~~<br>___/~~_____/~~____/~~_/~~___/~~_/~~<br>__/~~____/~~~~~~___/~~~~~_____/~~~~~<br></pre>",
      "<span style='color:#009966;'><pre>█████████████████████████████████████<br>█████████████████████████████████████<br>████ ▄▄▄▄▄ █▀▄█▀ ▀▄ █▀█▄ █ ▄▄▄▄▄ ████<br>████ █   █ █▄   ▄▀ ▀▀▄▀  █ █   █ ████<br>████ █▄▄▄█ █ ▀█▀█▄▄ ▄▄▄ ██ █▄▄▄█ ████<br>████▄▄▄▄▄▄▄█ ▀▄█ █ ▀ ▀▄█▄█▄▄▄▄▄▄▄████<br>████ ▄▀▄█▀▄▄▀█ ▀█ ▀██▄▀█▀█▄▄▄▀▄▄▀████<br>████  ▀▄▀█▄█▀▄▄██ ██ ▄▄▄▄▀ ▄▀█▀▀█████<br>████▀█▀ ▀ ▄ ▄█▀▀  █▀█  ▀▀▄ ██▀█ ▄████<br>████▄▄▄▀▄▄▄▄▀▄██  ▄█▄▀▄█ ▄ ▄███ ▄████<br>████▄▀▀█▄▄▄▄▄██▄█ ▀  █▀▄▄█▀ ▄▀█▀▄████<br>████▄█▀███▄▀▄ █▀█▄ █▄ █▀▄  ▄█▄▄▄ ████<br>████▄████▄▄▄ ▀    ▄█▀█▀█ ▄▄▄ ▄▄▀█████<br>████ ▄▄▄▄▄ █▄▄▀  ▀███▄▄▀ █▄█ ▀  █████<br>████ █   █ █▀█▀ ▄  ▀▄██▀ ▄  ▄▄█ █████<br>████ █▄▄▄█ █▀█▀ ▀▀ ██ █▀▀▄██▄██▀▄████<br>████▄▄▄▄▄▄▄█▄▄▄▄██▄█▄▄▄█▄▄██▄▄█▄█████<br>█████████████████████████████████████<br>█████████████████████████████████████<br></pre>",
      "<span style='color:#009966;'><pre>+---[RSA 4096]----+<br>|#E*....          |<br>|%#++ .           |<br>|@*O.=            |<br>|O*.B . .         |<br>|*o. o . S        |<br>|oo . . .         |<br>|o .   .          |<br>|.                |<br>|                 |<br>+----[SHA256]-----+<br></pre>",
    ],
    contentType: "html",
    typeSpeed: 3,
    loop: !0,
    shuffle: !0,
    showCursor: !1,
    autoInsertCss: !1,
    smartBackspace: !1,
  }),
  styles;
function enableDrag() {
  const t = document.querySelectorAll(".draggable"),
    e = [];
  for (let n = 0; n < t.length; n++) {
    const s = t[n],
      o = new Draggabilly(s, {});
    e.push(o);
  }
  function n(t) {
    const n = e.length;
    for (let s = 0; s < n; s++) t.matches ? e[s].disable() : e[s].enable();
  }
  const s = window.matchMedia("screen and (max-width: 600px)");
  n(s), s.addListener(n);
}
enableDrag();
function startJitter() {
  const e = document.querySelector("#cardcontent"),
    t = document.querySelector("html");
  function s() {
    (e.style.textShadow = "4px 0px 1px"), (t.style.mixBlendMode = "luminosity");
  }
  function o() {
    (e.style.textShadow = "0px 0px 1px"), (t.style.mixBlendMode = "normal");
  }
  function n() {
    const e = Math.random() * 9e3 + 4e3;
    setTimeout(s, e), setTimeout(o, e + 200), setTimeout(n, 5e3);
  }
  n();
}
startJitter();
function scanlines() {
  var o = document.getElementById("line").offsetHeight,
    r = document.getElementById("dot").offsetWidth,
    a = 0,
    m = document.getElementById("line").style.top,
    h = document.getElementById("dot").style.left,
    t = 3e3,
    s = t / 3,
    i = 2e3,
    e = window.innerHeight || document.documentElement.clientHeight,
    n = window.innerWidth || document.documentElement.clientWidth,
    c = e - (o + a),
    l = n - r;
  document.getElementById("cover").style.height = e + "px";
  function d() {
    var s = document.getElementById("line"),
      n = null;
    function o(e) {
      n || (n = e);
      var a = e - n;
      (s.style.top = (a / t) * c + "px"),
        a < t
          ? window.requestAnimationFrame(o)
          : ((s.style.top = m), (n = null), setTimeout(d, i));
    }
    window.requestAnimationFrame(o);
  }
  function u() {
    var n = document.getElementById("dot"),
      t = null;
    function o(e) {
      t || (t = e);
      var a = e - t;
      (n.style.left = (a / s) * l + "px"),
        a < s
          ? window.requestAnimationFrame(o)
          : ((n.style.left = h), (t = null), setTimeout(u, i));
    }
    window.requestAnimationFrame(o);
  }
  d(),
    u(),
    window.addEventListener(
      "resize",
      function () {
        (e = window.innerHeight || document.documentElement.clientHeight),
          (n = window.innerWidth || document.documentElement.clientWidth),
          (c = e - (o + a)),
          (l = n - r),
          (document.getElementById("cover").style.height = e + "px");
      },
      { passive: !0 },
    );
}
scanlines();
function startScreenGlow() {
  function e() {
    document.querySelector("html").style.filter =
      "-webkit-filter: blur(0px)  saturate(10)";
  }
  function t() {
    setTimeout(function () {
      document.querySelector("html").style.filter = "none";
    }, 114200);
  }
  function s() {
    const s = 1500,
      n = 100;
    setTimeout(e, 0),
      setTimeout(t, n),
      setTimeout(e, n * 2),
      setTimeout(t, s),
      setTimeout(e, s + n),
      setTimeout(t, s + n * 2);
  }
  function n() {
    const e = 1e4,
      t = 2e4,
      o = Math.random() * (t - e) + e;
    setTimeout(s, o), setTimeout(n, 35e3);
  }
  n();
}
startScreenGlow(),
  (styles = [
    "background: black",
    "font: monospace",
    "font-size: 20pt",
    "color: #009966",
    "display: block",
    "background-size: 100%",
    "text-align: left",
    "background-padding: 20%",
    "font-weight: bold, border:   border: 0px solid #009966",
  ].join(";")),
  console.log(
    `%c  ██████╗ ██████╗     ██████╗ 	
██╔════╝ ╚════██╗   ██╔═████╗	
██║█████╗ █████╔╝   ██║██╔██║	
██║╚════╝██╔═══╝    ████╔╝██║	
╚██████╗ ███████╗██╗╚██████╔╝	
 ╚═════╝ ╚══════╝╚═╝ ╚═════╝ 	
`,
    styles,
  ),
  (function () {
    var e = [
        {
          text: "<q>Part of the inhumanity of the computer is that, once it is competently programmed and working smoothly, it is completely honest.</q><br />- Eleanor Roosevelt",
        },
        {
          text: "<q>The Internet is the first thing that humanity has built that humanity doesn&#8217;t understand, the largest experiment in anarchy that we have ever had.</q><br />- Eric Schmidt",
        },
        {
          text: "<q>It seems probable that once the machine thinking method had started, it would not take long to outstrip our feeble powers... They would be able to converse with each other to sharpen their wits. At some stage therefore, we should have to expect the machines to take control.</q><br />- Alan Turing",
        },
        {
          text: "<q>UNIX is basically a simple operating system, but you have to be a genius to understand the simplicity.</q><br />- Dennis Ritchie",
        },
        {
          text: "<q>Any sufficiently advanced technology is indistinguishable from magic.</q><br />- Arthur C. Clarke",
        },
        {
          text: "<q>The only truly secure system is one that is powered off, cast in a block of concrete and sealed in a lead-lined room with armed guards.</q><br />- Gene Spafford",
        },
        {
          text: "<q>I think computer viruses should count as life. I think it says something about human nature that the only form of life we have created so far is purely destructive. We&#8217;ve created life in our own image.</q><br />- Stephen Hawking",
        },
        {
          text: "<q>Computers are useless. They can only give you answers.</q><br />- Pablo Picasso",
        },
        {
          text: "<q>If it works, it&#8217;s beautiful.</q><br />- Stephen Hawes",
        },
        {
          text: "<q>When it comes to their capacity to screw things up, computers are becoming more human every day.</q><br />- Seth Lloyd",
        },
      ],
      t = e[Math.floor(Math.random() * e.length)];
    document.getElementById("quote").innerHTML =
      "<aside>" + t.text + "</aside>";
  })();
function glitch() {
  const { startGlitch: e, stopGlitch: t } = PowerGlitch.glitch("a, .dropbtn", {
    playMode: "hover",
    createContainers: !0,
    hideOverflow: !0,
    timing: { duration: 100, iterations: 1, easing: "ease-out" },
    glitchTimeSpan: { start: 0, end: 1 },
    shake: { velocity: 0, amplitudeX: 0, amplitudeY: 0 },
    slice: {
      count: 10,
      velocity: 20,
      minHeight: 0.02,
      maxHeight: 0.3,
      hueRotate: !1,
    },
  });
}
setTimeout(glitch, 0);
function handleTabFocus() {
  const e = document.querySelector("link[rel~='icon']"),
    t = document.title;
  (window.onblur = () => {
    (e.href = "favicon_off.ico"), (document.title = "[sleeping...]");
  }),
    (window.onfocus = () => {
      (e.href = "favicon.ico"), (document.title = t);
    });
}
handleTabFocus(),
  fetch("https://api.ipify.org?format=jsonp&callback=")
    .then((e) => e.text())
    .then((e) => {
      const t = e.indexOf("{"),
        n = e.lastIndexOf("}"),
        s = JSON.parse(e.substring(t, n + 1));
      (ipinfo = s),
        ipinfo.ip != null &&
          ((document.querySelector(".ipaddress").innerHTML = ipinfo.ip),
          (document.querySelector(".ipinfo").style.display = "revert"));
    })
    .catch((e) => console.log(e)),
  document.addEventListener(
    "DOMContentLoaded",
    function () {
      var t = document.getElementById("cover"),
        e = Array.from(document.getElementsByClassName("draggable"));
      shuffleArray(e),
        (t.style.opacity = "0"),
        (t.style.transition = "opacity 0.5s ease"),
        setTimeout(function () {
          t.style.opacity = "1";
        }, 250),
        setTimeout(function () {
          for (var t = 0; t < e.length; t++)
            (e[t].style.display = "block"),
              (e[t].style.opacity = "0"),
              (e[t].style.transition =
                "opacity 0.5s ease, transform 0.5s ease"),
              (e[t].style.transform = "translateY(-100px)"),
              setTimeout(
                function (t) {
                  (e[t].style.opacity = "1"),
                    (e[t].style.transform = "translateX(0)");
                },
                50 * t,
                t,
              );
          setTimeout(function () {
            e.forEach(function (e) {
              e.style.transition = "none";
            });
          }, e.length * 100);
        }, 1e3);
    },
    { passive: !0 },
  );
function shuffleArray(e) {
  for (var n, s, t = e.length - 1; t > 0; t--)
    (n = Math.floor(Math.random() * (t + 1))),
      (s = e[t]),
      (e[t] = e[n]),
      (e[n] = s);
}
function flashElement(e, t, n) {
  const s = document.getElementById(e),
    a = s.style.color,
    r = s.style.backgroundColor,
    c = "#096",
    l = "black";
  let o = 0;
  function i() {
    (s.style.color = c),
      (s.style.backgroundColor = l),
      setTimeout(() => {
        (s.style.color = a),
          (s.style.backgroundColor = r),
          o++,
          o < t && setTimeout(i, n);
      }, n);
  }
  i();
}
document.getElementById("about-link").addEventListener(
  "click",
  function () {
    flashElement("about", 5, 100);
  },
  { passive: !0 },
);
const modal = document.getElementById("ErrModal"),
  openModalBtns = document.querySelectorAll(".openModalBtn");
function openModal() {
  (modal.style.display = "block"), (modal.style.opacity = "1");
}
function closeModal() {
  (modal.style.opacity = "0"),
    setTimeout(function () {
      (modal.style.display = "none"), (modal.style.opacity = "0");
    }, 300);
}
openModalBtns.forEach(
  function (e) {
    e.addEventListener("click", openModal);
  },
  { passive: !0 },
),
  window.addEventListener(
    "click",
    function (e) {
      e.target === modal && closeModal();
    },
    { passive: !0 },
  );
const textWrap = document.getElementById("wrap-lines");
textWrap.addEventListener(
  "click",
  function () {
    document.body.classList.contains("no-wrap")
      ? document.body.classList.remove("no-wrap")
      : document.body.classList.add("no-wrap");
  },
  { passive: !0 },
);
const highlightRegions = document.getElementById("highlight-regions");
highlightRegions.addEventListener(
  "click",
  function () {
    draggableElems = document.getElementsByClassName("draggable");
    for (let e = 0; e < draggableElems.length; e++)
      draggableElems[e].classList.contains("sepia")
        ? draggableElems[e].classList.remove("sepia")
        : draggableElems[e].classList.add("sepia");
  },
  { passive: !0 },
);
const shellType = document.getElementById("shell-type");
shellType.addEventListener(
  "click",
  function () {
    document.querySelector("html").style.cssText =
      "filter: hue-rotate(" + Math.floor(Math.random() * 361) + "deg)";
  },
  { passive: !0 },
);
const executeScript = document.getElementById("execute-script");
executeScript.addEventListener(
  "click",
  function () {
    (footer = document.getElementById("footer")),
      (defaultFooterText = footer.innerText),
      (footer.style.cssText = "filter: invert(1)"),
      (footer.innerHTML = "executing..."),
      setTimeout(function () {
        (footer.innerHTML = defaultFooterText),
          (footer.style.cssText = "filter: invert(0)");
      }, 2e3);
  },
  { passive: !0 },
);
const copyButton = document.getElementById("copyButton");
copyButton.addEventListener(
  "click",
  function () {
    (iframeCopy = document.getElementById("iframe-copy")),
      (htmlContent = `
    <div class="move card" id="added-copy">
      <iframe
        src="https://theden.sh"
        width="200"
        height="300"
        scrolling="no"
        frameborder="0"
        style="padding: 1.2em 0.5em 0.7em 0.5em;"
      ></iframe>
    </div>
`),
      (iframeCopy.innerHTML = htmlContent);
  },
  { passive: !0 },
);
const cutButton = document.getElementById("cutButton");
cutButton.addEventListener(
  "click",
  function () {
    (iframeCopy = document.getElementById("added-copy")),
      iframeCopy !== null && iframeCopy.remove();
  },
  { passive: !0 },
);
